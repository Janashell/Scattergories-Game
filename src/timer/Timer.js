...
const Timer = ({ setHide }) => {
    ...
    // Function that begins the coundown and reveals text/content for list items (AKA Categories)
     const playGame = () => {
        setTime (time - 1);
        setHide (false);
    }
        ...
        if (time !== startingTime) {
            ...
            setTimeout (() => {
                ...
                if (time > 0) {
                    ...
                    // Hides the list item text/content on all categories (AKA List items) by adding the "hide" CSS class to all of them
                    setHide (true);
                    ...
                    setTimeout (() => {
                        alert ('TIME IS UP!!');
                        // After the alert is complete (AKA The user clicks "OK") the categories are shown again
                    }, 100);
                    }
            }, 1000);
        }
